...
```sql
createtablet7（
idintPRIMARYKEYauto_increment，#主键约束 name varchr(20)
。notnul:不为空，当你设置一个学段时，不充许它为空，可以使用此约束条件
createtabletl
idintPRIMARYKEYauto_increment，#主键约束
namevarchar（2o）notnull#name字段的值不可以为空

·default默认值
。例如：对于性别一列，如果大部分都是男性，可以设置成默认值，不填则取默认值，填写了则覆盖默认值

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk5MTY5MzA1MSwtNDk5OTU5NDcyLC00OD
c5NTI0NjUsLTE3MDA1OTI1ODgsMTQ4ODczNTg3MywtMTkwMDA4
ODU1LDM2MTM3MDA4NSw2OTczMTUxOTksLTM2OTk0ODMxMywyNT
c3NDkyMDQsLTExNjcxMTIyMiwtNTI0NzcyNzIsLTUxODE2Mzgx
NywxNzIwMDQ1MzcsLTE5NDE1MzA4LDI2NTU2MTI0LC0xNTgyNz
E2MDEyLDY1MTI4NzcyNSwtOTAwNzY3OTIwLC05OTg1Mjc0XX0=
