...
for url in urls:  
    c = get_request(url)  
    task = asyncio.ensure_future(c)  
    task.add_done_callback(parse)  # 回调函数 :当我的任务对象成功执行之后，才会回头执行parse函数
    tasks.append(task)  
​  
loop = asyncio.get_event_loop()  
loop.run_until_complete(asyncio.wait(tasks))  
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU0NDA4ODEyMywxMjgyODczODg1LDE1Mz
U4NzQ2NDMsLTIxMTcxODM4MjMsMTg3NTU3MDQ2MCwtMTgyMDM2
MTEwNCwtNDc5MzM3MzI1XX0=
-->